...
| **geo-press-review** | Compare external press (Google News/web) vs what's on Geo → ranked "publish next" list (already on Geo / not on Geo yet). Recommends only. | Repo (coverage script + web search) |

### actionable/ — changes Geo, editors only

| Skill | What it does | Runs in |
|---|---|---|
| **geo-publish** | Create/update/delete entities and relations via the GRC-20 SDK — entities, claims, types, and all page-block content (text, media, data/collection/query/ranking, tabs). Safeguards before any write. | Repo + wallet key |
